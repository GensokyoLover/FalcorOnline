...
def initialize_markov_chain(patch, scene_path, tonemap):
    global renderer
    renderer = FalcorVariableRenderer(tonemap_type=tonemap,deviceID = 2)
...
if __name__ == '__main__':
    ...
    for i in range(conf.num_patches*uniform_multiple_variable):
        mc_uniform_samplers.append(MarkovChainSampler(large_step_prob=1.0, mutation_size_large=conf.mutation_size_large, mutation_size_small=conf.mutation_size_small, dimensions=renderer.scene.total_parameters() + 2))
    ...
    if conf.arch == 'pixel':
        ...
        model = MoE(12,buffers_features=13,variables_features=renderer.scene.total_parameters(),hidden_features=conf.hidden_features,hidden_layers=conf.hidden_layers)
